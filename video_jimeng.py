...
# ---------------------------------------------------------------------------
# Backend route for Jimeng model fetching
# ---------------------------------------------------------------------------
try:
    ...
    if (_comfy_server is not None
        ...
        @_routes.post("/zhangyuapi_fetch_jimeng_models")
        async def _zhangyuapi_fetch_jimeng_models_route(request):
            try:
                ...
                jimeng_models = _filter_jimeng_models(all_models)
                return _aiohttp_web.json_response(
                    {"status": "success", "models": jimeng_models},
                )
            except RuntimeError as exc:
                msg = str(exc)
                print(f"[Jimeng] fetch models error: {msg}")
                return _aiohttp_web.json_response(
                    {"status": "error", "message": msg}, status=502,
                )
            except Exception as exc:
                print(f"[Jimeng] fetch models error: {exc}")
                return _aiohttp_web.json_response(
                    {"status": "error", "message": str(exc)}, status=500,
                )
except Exception as _exc:
    print(f"Warning: Could not register Jimeng model-fetch route: {_exc}")
    ...
    def generate(self, **kwargs):
        ...
        try:
            model, model_list = resolve_and_validate_model(
                ...
                placeholder="jimeng",
                filter_func=_filter_jimeng_models,
            )
